...
export default function CalendrierScreen() {
  ...
  const [sousOnglet, setSousOnglet] = useState<SousOnglet>('moi');
        ...
        <>
          <View className="mb-2 flex-row rounded-xl bg-slate-100 p-1 mx-4">
            {SOUS_ONGLETS.map((so) => (
              <Pressable
                key={so.value}
                onPress={() => setSousOnglet(so.value)}
                className={`flex-1 items-center rounded-lg py-1.5 ${sousOnglet === so.value ? 'bg-white' : ''}`}
              >
                <Text
                  className={`text-xs ${sousOnglet === so.value ? 'font-semibold text-indigo-600' : 'text-slate-500'}`}
                  numberOfLines={1}
                >
                  {so.label}
                </Text>
          ...
          {pasDePopUp ? (
            ...
            <>
              {sousOnglet === 'tous' && popUps && popUps.length > 0 && (
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                    <View style={{ flexDirection: 'row', gap: 8 }}>
                      {jours.map((jour) => {
                        ...
                        const reglesJour = horairesPourVue.filter((h) => h.jour_semaine === jourIso && h.actif);
                        ...
                        const heureFermeture = ouvert
                          ...
                          : '20:00:00';
                        const shiftsJourPublies = (shifts ?? []).filter((s) => s.date === dateIso && s.statut === 'publie');
                        const shiftsJour =
                          sousOnglet === 'moi'
