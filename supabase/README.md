...
15. Dans l'app (écran Pop-up), marquer un lieu comme "C'est le local" **avant** l'étape suivante, sinon rien n'aura de local à cibler. Puis coller et exécuter le contenu de `migrations/0016_horaire_defaut_admins.sql` (donne à chaque admin un horaire par défaut au local de 10h à 19h du lundi au samedi, éditable ensuite dans Équipe comme pour n'importe qui — un trigger applique automatiquement le même horaire par défaut à toute future promotion en admin).
